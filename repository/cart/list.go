...
func (r *repo) List(reqQuery *cartModel.ListReqQuery) (resData []cartModel.ListCartResData, count int64, err error) {
	...
	return resData, count, r.db.Select("id, user_id").Model(&cartModel.Cart{}).Where("status = ?", reqQuery.Status).Count(&count).Limit(reqQuery.Limit).Offset(reqQuery.Offset).Scan(&resData).Error
}
